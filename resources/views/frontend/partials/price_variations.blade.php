                    ...
                    @if ($option->type == 'radio' || $option->type == 'select')
                        @foreach ($option->options as $options)
                            <div class="form-group clearfix row">
                                ...
                                <div class="col-sm-4">
                                    <input class="form-control" type="number" lang="en" min="0" step="0.01"
                                        name="{{ $option->name }}_{{ $options }}_price" required>
                                </div>
                                <div class="col-sm-4">
                                    <select class="form-control selectpicker"
                                        name="{{ $option->name }}_{{ $options }}_variation">
                                        <option value="increase">{{ __('front.Increase') }}</option>
                                        <option value="decrease">{{ __('front.Decrease') }}</option>
                                    </select>
                                </div>
                            </div>
                        @endforeach
