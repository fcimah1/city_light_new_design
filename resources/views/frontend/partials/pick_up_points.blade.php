<div class="card-header bg-white py-3">
    <h5 class="heading-6 mb-0">{{ __('front.Select Nearest Pick-up Point') }}</h5>
</div>
@php
$admin_products = [];
$seller_products = [];
...
@if (!empty($admin_products))
    @foreach ($pick_up_points as $key => $pick_up_point)
        <label class="mega-radio w-100">
            ...
            <span class="d-block">
                <br><strong>{{ __('front.Address') }}: {{ $pick_up_point->getTranslation('name') }}</strong>
                <br><strong>{{ __('front.Address') }}: {{ $pick_up_point->getTranslation('address') }}</strong>
                <br><strong>{{ __('front.Phone') }}: {{ $pick_up_point->phone }}</strong>
            </span>
        </label>
    @endforeach
@endif
