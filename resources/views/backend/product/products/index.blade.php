                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="lg">{{ translate('Hot Offer') }} </th>
                    ...
                    <tbody>
                        @foreach ($products as $key => $product)
                            <tr>
                                ...
                                <td>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input onchange="update_best(this)" value="{{ $product->id }}" type="checkbox"
    ...
    <script type="text/javascript">
        ...
        function update_todays_deal(el) {
            ...
            $.post('{{ route('products.todays_deal') }}', {
                ...
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Hot Offer updated successfully') }}');
                } else {
        ...
        function update_best(el) {
            ...
            $.post('{{ route('products.best') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Best Seller products updated successfully') }}');
                } else {
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
            });
        ...
        function update_featured(el) {
            ...
            $.post('{{ route('products.featured') }}', {
                ...
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('New Arrival products updated successfully') }}');
                } else {
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
            });
