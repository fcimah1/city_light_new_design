...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{translate('All Sub Categories')}}</h1>
        </div>
...
<div class="card">
    <div class="card-header d-block d-md-flex">
        <h5 class="mb-0 h6">{{ translate('SubCategories') }}</h5>
            ...
            <tbody>
                @foreach($subCategories as $key => $subCategory)
                    <tr>
                        ...
                        <td>
                            @php
                                $parent = \App\Models\Category::where('id', $subCategory->category_id)->first();
                            @endphp
                            @if ($parent != null)
                        ...
                        <td>
                            @if($subCategory->banner != null)
                                <img src="{{ uploaded_asset($subCategory->banner) }}" alt="{{translate('Banner')}}" class="h-50px">
                            @else
                                —
                            @endif
                        </td>
                        <td>
                            @if($subCategory->icon != null)
                                <span class="avatar avatar-square avatar-xs">
                                    <img src="{{ uploaded_asset($subCategory->icon) }}" alt="{{translate('icon')}}">
                                </span>
                            @else
                                —
                            @endif
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_featured(this)" value="{{ $subCategory->id }}" <?php if($subCategory->featured == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
                        <td>{{ $subCategory->commision_rate }} %</td>
                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('subcategories.edit', ['id'=>$subCategory->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
    ...
    <script type="text/javascript">
        function update_featured(el){
            ...
            $.post('{{ route('subcategories.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Featured subCategories updated successfully') }}');
                }
                else{
