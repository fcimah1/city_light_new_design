...
if(!function_exists('last_flash')){
    function last_flash(){
        ...
        $flash = \App\Models\FlashDeal::where('status', 1)
            ->where('start_date', "<=", $today)
            ->where('end_date', ">", $today)
            ->orderBy('created_at', 'desc')
            ->take(2)->get();
        return (count($flash)>0)? $flash :array();
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($product, $formatted = true,$symbol=true)
    {
        ...
        return $formatted ? format_price(convert_price($price),$symbol) : $price;
    }
}
...
if (!function_exists('trimString')) {
    function trimString($inputString, $length = 50)
    {
